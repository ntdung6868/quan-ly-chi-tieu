...
async function exportExcel(transactions: Transaction[], dateRange: { start: string; end: string }, presetLabel: string) {
  ...
  // Summary rows - chỉ dùng cột C và D
  ws.addRow([]);
  ...
  // Borders chỉ cho vùng có data (table header = row 8, data rows sau đó)
  const tableHeaderRow = 8; // 1 title + 5 info + 1 empty + 1 header
